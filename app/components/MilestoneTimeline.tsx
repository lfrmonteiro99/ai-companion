interface Milestone { type: string; label: string; createdAt: Date; }

export default function MilestoneTimeline({ milestones }: { milestones: Milestone[] }) {
  return (
    <section className="mb-8">
      <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-[var(--text-muted)]">Milestones</h2>
        ...
        {milestones.map((m, i) => (
          <div key={`${m.type}-${i}`} className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-[var(--text-faint)]">{new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" }).format(new Date(m.createdAt))}</p>
            </div>
          </div>
        ))}
      </div>
