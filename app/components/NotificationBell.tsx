...
export default function NotificationBell({ userId }: { userId: string | null }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen(!open)} className="relative rounded-lg p-2 text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-elevated)] hover:text-[var(--text-primary)]">
        <Bell size={18} />
        {unreadCount > 0 && (
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-80 rounded-xl surface-2 animate-fade-in">
          <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3">
            ...
            {unreadCount > 0 && <button onClick={markAllRead} className="text-xs text-[var(--agent-accent)] hover:brightness-125 transition">Mark all read</button>}
          </div>
          <div className="max-h-80 overflow-y-auto">
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => (
                <a key={n.id} href={`/chat/${n.agentId}`} onClick={() => markAsRead(n.id)} className={`block border-b border-[var(--border)] px-4 py-3 transition-colors hover:bg-[var(--bg-elevated)] ${!n.read ? "bg-[var(--agent-subtle)]" : ""}`}>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-[var(--text-primary)]">{n.title}</span>
                    {!n.read && <span className="h-1.5 w-1.5 rounded-full bg-[var(--agent-accent)]" />}
                  </div>
                  <p className="mt-0.5 text-xs text-[var(--text-muted)]">{n.body}</p>
                  <p className="mt-1 text-[10px] text-[var(--text-faint)]">{new Date(n.createdAt).toLocaleString()}</p>
                </a>
              ))
            )}
          </div>
